...
using unode = std::unique_ptr<LLNode>; //unique pointer to LLNode
using str = std::string;

LLNode::LLNode()   //default constuctor
    : head('\0'), label(""), eow(0), rightsib(nullptr), child1(nullptr){printf("Creating node with default constructor.\n");}

LLNode::LLNode(char head_in)   //default constuctor
    : head(head_in), label(""), eow(0), rightsib(nullptr), child1(nullptr){printf("Creating node with char constructor.\n");}
...
{//returns ptr to the sibling of this node instance or the instance itself if its head equals the target_head
    ...
    this->print();
    snode targetnode = std::make_shared<LLNode>(*this);  //FIX LATER//
...
{//prints the sibling nodes from the target_node down the line
    ...
    this->print();
    snode targetnode = std::make_unique<LLNode>(*this);  //FIX LATER//
    while(targetnode != nullptr)
    {//while the end of the sibling chain hasn't bee reached.
        targetnode->print();
        targetnode = targetnode->rightsib;
...
void LLNode::edit(str new_word, bool e_o_w)
{//edits node instance
    this->head = new_word[0];
    this->label = new_word.substr(1);
    if(e_o_w)
        this->eow = 1;
}
...
//     target->leftsib->rightsib = nullptr;
//     target->rightsib->leftsib = nullptr;
// // }
